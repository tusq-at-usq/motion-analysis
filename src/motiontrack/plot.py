...
class PlotMatch:
    def __init__(self, name, resolution=None):
        ...
        self.view.addItem(self.dot_ob)
        self.view.addItem(self.dot_pr)
        self.view.addItem(self.aruco_ob)
        self.view.addItem(self.aruco_pr)
        self.view.addItem(self.dot_CoM)
        self.view.addItem(self.frame_pr)
        self.view.addItem(self.edges)

        self.view.addItem(self.vec1)
        self.view.addItem(self.vec2)
        self.view.addItem(self.vec3)
    ...
    def update_CoM(self,dot_cent):
        self.dot_CoM.setData(dot_cent[0], dot_cent[1])
        self.app.processEvents()

    def update_dot_observation(self, dot_r):
        self.dot_ob.setData(dot_r[0], self.invert(dot_r[1]))
        self.app.processEvents()

    def update_aruco_observation(self, aruco_r):
        self.aruco_ob.setData(aruco_r[0], self.invert(aruco_r[1]))
        self.app.processEvents()

    def update_dot_projection(self, dot_data):
        dot_r = dot_data.points
        self.dot_pr.setData(dot_r[:,0], self.invert(dot_r[:,1]))
        self.app.processEvents()

    def update_aruco_projection(self, aruco_data=None):
        self.aruco_pr.setData(aruco_data[:,0], self.invert(aruco_data[:,1]))
        self.app.processEvents()
    ...
    def update_vectors(self, body_vecs):
        self.vec1.setData(body_vecs[0][0], self.invert(body_vecs[0][1]))
        self.vec2.setData(body_vecs[1][0], self.invert(body_vecs[1][1]))
        self.vec3.setData(body_vecs[2][0], self.invert(body_vecs[2][1]))

    def close(self):
        self.view.close()

    def export(self, prefix='unnamed', index=0, DIRECTORY="./"):
        exporter = pg.exporters.ImageExporter( self.view.scene() )
        #  exporter = pg.exporters.ImageExporter(self.app)
        exporter.export(DIRECTORY+'/exports/'+prefix+'_'+str(index)+'.png')
...
class PlotTrack:
    def __init__(self, x_dict):
        ...
        self.p5 = self.win.addPlot(title="Acceleration")
    ...
    def update_state(self, x, t):
        ...
        self.a_x.setData(t, x[self.x_dict['a_x']])
        self.a_y.setData(t, x[self.x_dict['a_y']])
        self.a_z.setData(t, x[self.x_dict['a_z']])
    ...
    def load_smoothed_data(self, x_smoothed, t):
        ...
        t = np.array(t)
        pen = pg.mkPen(0.2, style=QtCore.Qt.PenStyle.DashLine, width=1)
