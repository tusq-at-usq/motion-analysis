...
class Physics6DoF(Block):
    ...
    def __init__(self):
        metadata = Metadata(states=[
            "x", "y", "z", "q0", "q1", "q2", "q3", "v_x", "v_y", "v_z", "p",
            "q", "r"
        ],
                            inputs=[],
                            outputs=[],
                            parameters=["mass", "I_11", "I_2", "I_33", "I_13"])
        super().__init__(metadata)

    def compute_dynamics(self, t, states, algebraics, inputs, parameters):
        _, _, _, q0, q1, q2, q3, v_x, v_y, v_z, p, q, r = states
        _, I_11, I_2, I_33, I_13 = parameters

        # From Zipfel eq. 10.18
        p_dot = q / (I_11 * I_33 - I_13**2) * (
            (I_2 * I_33 - I_33**2 - I_13**2) * r - I_13 *
            (I_33 + I_11 - I_2) * p)

        q_dot = 1. / I_2 * ((I_33 - I_11) * p * r + I_13 * (p**2 - r**2))

        r_dot = q / (I_11 * I_33 - I_13**2) * (
            (-I_11 * I_2 + I_11**2 + I_13**2) * p + I_13 *
            (I_33 + I_11 - I_2) * r)

        lambd = 1 - (q0**2 + q1**2 + q2**2 + q3**2)
        k = 0.5
        correction = sp.Matrix([[lambd * k], [lambd * k], [lambd * k],
                                [lambd * k]])
        Q = sp.Matrix([[q0], [q1], [q2], [q3]])
        dq_matrix = 0.5 * sp.Matrix([[0, -p, -q, -r],
                                     [p, 0, r, -q],
                                     [q, -r, 0, p],
                                     [r, q, -p, 0]])
