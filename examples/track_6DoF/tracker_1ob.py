...
def test_tracking_loop(print_flag=0):
    ...
    while t_next < np.inf:
        ...
        dt = t_next - t
        A = dsys.J_np(x,[])
        Q = custom_process_noise(A, Q_c, dt, 100)
    ...
    ax[0].plot(t_history, y1,'o')
    ax[0].plot(t_history, y1_true,'-',color='k')
    ax[0].plot(t_history, y1_tracked,'-',color='r')
    ax[0].set_title('Observable 1')

    ax[1].plot(t_history, y2,'o')
    ax[1].plot(t_history, y2_true,'-',color='k')
